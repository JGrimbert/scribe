<template>
  <div class="conformity">
    <p class="conformity-lead">
      <strong>{{ conformity.conformCount }}</strong> chapitre(s) conforme(s) sur
      {{ conformity.leafCount }} — {{ formatPercent(rate) }}.
...
const conformity = computed(() => analysis.value?.conformity ?? { conformCount: 0, leafCount: 0, criteria: [] })

const rate = computed(() =>
  conformity.value.leafCount ? conformity.value.conformCount / conformity.value.leafCount : 0,
)
...
const option = computed(() => {
  ...
  return {
    ...
    xAxis: { type: 'value', max: conformity.value.leafCount || 1, show: false },
    yAxis: {
      type: 'category',
      inverse: true,
