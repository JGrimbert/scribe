...
function updateDimensions() {
  ...
  docBarHeight.value = docBarEl ? docBarEl.offsetHeight : 0
  menuHeight.value = menuEl ? menuEl.offsetHeight : 0

  // Position de la track : entre le bas de doc-bar+menu et le bas de la fenêtre
  trackTop.value = docBarHeight.value + menuHeight.value
  trackHeight.value = window.innerHeight - trackTop.value

  // Calcul de la hauteur du thumb (proportionnelle au contenu visible)
  // Ratio = hauteur visible / hauteur totale
  const ratio = containerHeight.value / contentHeight.value
  thumbHeight.value = Math.max(20, Math.min(trackHeight.value, trackHeight.value * ratio))
...
onMounted(() => {
  ...
  // Observer pour les changements de taille du contenu et du conteneur
